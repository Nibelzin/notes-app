...
import ConfigsModal from "../Modal/ConfigsModal";
import { useState } from "react";

export default function Header() {

    const handleConfigBoxClickOutside = () => {
        setConfigBoxVisible(false)
    }

    const configBoxRef = useDetectClickOutside({ onTriggered: handleConfigBoxClickOutside })
    const [configBoxVisible, setConfigBoxVisible] = useState(false)
    ...
    return (
        ...
        dark:bg-zinc-800 dark:text-white transition-colors">
            ...
            <div className="relative" ref={configBoxRef}>
                <button className="group/configs flex items-center gap-1" onClick={() => setConfigBoxVisible(!configBoxVisible)}>
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="opacity-0 group-hover/configs:opacity-100 size-4 transition-opacity">
                ...
                <ConfigsModal configBoxVisible={configBoxVisible} />
            </div>
        </div>
    )
}